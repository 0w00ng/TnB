...
const nav = [
  ["홈", "/"],
  ["디스코드 운영", "/systems/voice-ledger"],
  ["기록보관소", "/archive"],
];

const members = [
  ["OF", "김준범", "Original Founder", "TnB의 원조 설립자이자 최연장자. 가면과 지방이 잔뜩 껴있는 사람이라는 내부 평가를 품고도, 장난처럼 시작된 관계를 하나의 이름 아래 묶어낸 시작점이다."],
  ["BA", "김영웅", "Brand Architect", "인천 폰팔이 시절의 현장 감각을 바탕으로 B2B 영업사원까지 올라온 인물. 말도 안 되는 상황을 하나의 서사로 포장하고, 소동을 브랜드처럼 보이게 만든다."],
  ["CBO", "권우철", "Chief Brown Officer", "권위보다는 똥으로 기억되고, 리더십보다는 상징성으로 살아남은 내부 밈의 핵심 인물이다."],
  ["NA", "조윤성", "Noble Analyst", "귀공자 출신의 날카로운 분석가. 하지만 TnB의 천박함이 점점 물들어가고 있으며, 이상한 흐름을 가장 먼저 감지한다."],
  ["BR", "김지헌", "Brutal Realist", "헛소리를 가장 빠르게 잘라내는 현실주의자. 짧고 직설적인 말로 과열된 분위기의 균형을 잡는다."],
  ["DR", "남궁홍주", "Dark Romantic", "감정선이 깊고 예측하기 어려운 분위기를 가진 인물. 위험하지만 묘하게 빠져드는 존재감이 있다."],
  ["GO", "이병하", "Ground Operator", "현실의 무게를 아는 실전형 멤버. 과하게 나서지 않지만 필요한 순간에 반응하는 조용한 운영자다."],
  ["UH", "조성범", "Unpolished Heir", "여유 있는 배경과 다듬어지지 않은 너드미를 동시에 가진 멤버. 연애 시장에서는 아직 미출시 상태다."],
];

const modules = [
  ["창립 멤버 등록소", "TnB 멤버들의 역할, 별명, 캐릭터를 정리한 공식 인물 등록소입니다.", "#members", "Active"],
  ["디스코드 운영 시스템", "음성채널 체류시간과 활동 흐름을 실제 데이터로 관리하는 운영 시스템입니다.", "/systems/voice-ledger", "Beta"],
  ["내부 신화 기록소", "대화에서 파생된 사건과 내부 밈을 고급스러운 보고서처럼 보관합니다.", "/archive", "Active"],
  ["경쟁 운영 보드", "내전, 게임 기록, 순위표를 관리할 예정인 경쟁 운영 보드입니다.", null, "준비 중"],
];

const incidents = [
  ["T&B.inc 창립 선언", "김준범의 회사식 환영사로 단톡방이 조직 세계관으로 전환된 사건."],
  ["권우철 근태 위기", "참여 독촉이 정시출근, 인턴, office 방문 같은 회사식 근태 관리 문법으로 변질된 사건."],
  ["조윤성 승진 공지", "People Manager 임명과 승진 발표문으로 TnB 인사 체계가 생긴 순간."],
  ["채팅봇 궁합 폭주", "궁합 점수 하나로 멤버 관계도와 내부 밈이 동시에 재편된 봇 사고."],
  ["팀채팅 이전", "새 방, 새 로고, 새 방장 권한이 한 번에 재정의된 조직 이전 사건."],
  ["박다인 초대 사건", "외부 인원 초대로 private crew의 보안 감각이 가장 크게 흔들린 사건."],
];

const fallbackSummary = {
  todayTotalLabel: "1분 미만",
  weekTotalLabel: "1분 미만",
  mostActiveMember: "기록 없음",
  mostOccupiedChannel: "기록 없음",
...
function Logo({ className = "" }) {
  ...
  if (failed) {
    return <div className={`grid place-items-center rounded-full border border-[#d8b76f]/50 bg-[#120e09] font-black text-[#f4d993] shadow-[0_0_70px_rgba(216,183,111,0.24)] ${className}`}>T&B</div>;
  }
      ...
      <header className="sticky top-0 z-50 border-b border-[#d8b76f]/15 bg-[#030407]/82 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 sm:px-8 lg:px-10">
          <button onClick={() => go("/")} className="flex items-center gap-3 text-left">
            ...
            <div>
              <p className="text-sm font-black leading-none text-[#fff8e6]">T&B</p>
              <p className="mt-1 text-[10px] font-bold tracking-[0.18em] text-[#9e9176]">TRUST AND BELIEVE</p>
            </div>
          </button>
          <nav className="hidden items-center gap-2 md:flex">
            {nav.map(([label, href]) => (
              <button key={href} onClick={() => go(href)} className="rounded-full border border-white/10 bg-black/20 px-5 py-2.5 text-sm font-semibold text-[#f3ead8] transition hover:-translate-y-0.5 hover:border-[#d8b76f]/45 hover:bg-[#d8b76f]/10 hover:text-[#f5d58b]">
                {label}
              </button>
            ))}
          </nav>
    ...
    <Shell>
      <main>
        <section className="mx-auto grid max-w-7xl items-center gap-14 px-5 pb-24 pt-16 sm:px-8 md:grid-cols-[1.05fr_0.95fr] md:py-28 lg:px-10">
          <div>
            <Badge>비공식 크루 콘솔</Badge>
            <h1 className="mt-8 text-6xl font-black tracking-tight text-[#fff8e6] sm:text-7xl lg:text-8xl">T&B 운영체계</h1>
            <p className="mt-4 text-2xl font-semibold text-[#d8b76f]">Trust and Believe</p>
            <p className="mt-8 max-w-3xl text-3xl font-black leading-tight text-white sm:text-5xl">친구들의 소동과 결속을 <span className="block text-[#d7c7a1]">하나의 시스템으로.</span></p>
            ...
            <div className="mt-10 flex flex-col gap-4 sm:flex-row">
              <a href="#modules" className="rounded-2xl bg-[#d8b76f] px-6 py-4 text-center text-sm font-black text-[#15100a] shadow-[0_0_45px_rgba(216,183,111,0.22)] transition hover:-translate-y-1 hover:bg-[#f4d993]">콘솔 입장</a>
              <a href="#members" className="rounded-2xl border border-[#d8b76f]/35 bg-white/[0.03] px-6 py-4 text-center text-sm font-black text-[#fff8e6] transition hover:-translate-y-1 hover:border-[#f4d993]">멤버 보기</a>
            </div>
            <div className="mt-8 flex flex-wrap gap-3">{["시즌 2", "프라이빗 크루 콘솔", "소동 기반 운영"].map((x) => <Badge key={x} muted>{x}</Badge>)}</div>
          </div>
          <div className="relative mx-auto grid aspect-square w-full max-w-[500px] place-items-center">
            <div className="absolute inset-3 rounded-full border border-[#d8b76f]/20" />
            <div className="absolute inset-10 rounded-full border border-dashed border-[#d8b76f]/25" />
            <div className="absolute h-[74%] w-[74%] rounded-full bg-[#d8b76f]/10 blur-3xl" />
            <Logo className="relative h-[68%] w-[68%] border border-[#f4d993]/45 bg-[#120e09] text-5xl" />
            <div className="absolute bottom-4 rounded-full border border-[#d8b76f]/25 bg-[#05070b]/80 px-4 py-2 text-xs font-semibold text-[#d7c7a1] backdrop-blur">공식처럼 보이는 비공식 인장</div>
          </div>
        </section>

        <section id="modules" className="border-y border-[#d8b76f]/10 bg-[#07090f]/72">
          <div className="mx-auto max-w-7xl px-5 py-24 sm:px-8 lg:px-10">
            <p className="text-xs font-black uppercase tracking-[0.24em] text-[#d8b76f]">TnB Modules</p>
            <h2 className="mt-4 max-w-3xl text-4xl font-black text-[#fff8e6] sm:text-5xl">장난처럼 시작된 것들이 이제 시스템이 됩니다.</h2>
            <div className="mt-12 grid gap-5 md:grid-cols-2 xl:grid-cols-4">
              {modules.map(([title, desc, href, status]) => <Card key={title} className="flex min-h-[280px] flex-col"><div className="flex items-start justify-between gap-4"><h3 className="text-2xl font-black text-[#fff8e6]">{title}</h3><Badge muted={status === "준비 중"}>{status}</Badge></div><p className="mt-5 flex-1 text-sm leading-7 text-[#cfc6ad]/76">{desc}</p>{href ? <button onClick={() => href.startsWith("/") ? go(href) : document.querySelector(href)?.scrollIntoView({ behavior: "smooth" })} className="mt-7 w-fit rounded-2xl border border-[#d8b76f]/30 bg-[#d8b76f]/10 px-4 py-3 text-sm font-black text-[#f4d993]">열기</button> : <button disabled className="mt-7 w-fit cursor-not-allowed rounded-2xl border border-white/10 bg-white/[0.03] px-4 py-3 text-sm font-black text-[#8f856e]">준비 중</button>}</Card>)}
            </div>
          </div>
        </section>

        <section id="members" className="mx-auto max-w-7xl px-5 py-24 sm:px-8 lg:px-10">
          <p className="text-xs font-black uppercase tracking-[0.24em] text-[#d8b76f]">Founding Members Registry</p>
          <h2 className="mt-4 max-w-3xl text-4xl font-black text-[#fff8e6] sm:text-5xl">멤버는 친구가 아니라, 각자의 역할을 가진 조직 구성원입니다.</h2>
          <div className="mt-12 grid gap-5 md:grid-cols-2 xl:grid-cols-4">
            {members.map(([badge, name, role, desc]) => <Card key={name} className="min-h-[330px]"><div className="grid h-12 w-12 place-items-center rounded-full border border-[#d8b76f]/35 bg-[#1a1209] text-sm font-black text-[#f4d993]">{badge}</div><h3 className="mt-6 text-2xl font-black text-[#fff8e6]">{name}</h3><p className="mt-1 text-sm font-bold text-[#d8b76f]">{role}</p><p className="mt-4 text-sm leading-7 text-[#cfc6ad]/74">{desc}</p></Card>)}
          </div>
        </section>
      </main>
    </Shell>
...
function VoiceLedgerPage() {
  ...
  async function load() {
    try {
      const [s, r, recentRows, activeRows] = await Promise.all([fetchVoiceSummary(), fetchVoiceRanking(range), fetchRecentVoiceSessions(), fetchActiveVoiceSessions()]);
      setSummary(s); setRanking(r); setRecent(recentRows); setActive(activeRows); setConnected(true);
  ...
  useEffect(() => {
    let source;
    try { source = new EventSource(getVoiceLedgerEventUrl()); source.addEventListener("voice-ledger-update", load); } catch { source = null; }
